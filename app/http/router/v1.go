...
import (
	...
	"go_web/app/http/controller/my"
	"go_web/app/http/controller/upload"
...
func v1(router *gin.Engine) {
	...
	v1 := router.Group("/v1")
	{
		...
		authorized.Use(middleware.AuthHandler())
		{
			...
			authorized.POST("/chat_image", chat.Image)
		}

		v1.POST("/oss", upload.OssPost)
		myGroup := v1.Group("/my")
		myGroup.Use(middleware.AuthHandler())
		{
			myGroup.POST("/avatar", my.Avatar)
			myGroup.GET("/upload_policy_token", upload.PolicyToken)
		}
